...
class NFAfromRegex:
    ...
    def buildNFA(self):
        ...
        for char in self.regex:
            #情况一：遇到非运算符且在ASCII码里的拓展出来的字符集（数字或字母）
            if char in self.alphabet:
                #把字符加入语言表
                language.add(char)
                #如果前一个不是".",且前一个包含在字符集或者包含在["(","*"]
        ...
        #弹出所有的符号栈进行计算，最后NFA栈中的唯一NFA就是所求的NFA
    ...
    #带入运算符号直接进行计算，实现NFA栈弹出，合并，压栈的过程
